...
void TransactionGenerator::generate(Signer &signer) {
  if (signer.isNew()) {
    ...
    signature.insert(signature.cend(), new_transaction.transaction_id.cbegin(),
    ...
    signature.insert(signature.cend(), new_transaction.sent_time.cbegin(),
    ...
    signature.insert(signature.cend(), new_transaction.requestor_id.cbegin(),
    ...
    signature.insert(signature.cend(), transaction_type_str.cbegin(),
                     transaction_type_str.cend());

    auto user_id_str = to_string(signer.user_id);
    new_transaction.content_list.emplace_back(user_id_str);
    signature.insert(signature.cend(), user_id_str.cbegin(),
                     user_id_str.cend());

    new_transaction.content_list.emplace_back(signer.pk_cert);
    signature.insert(signature.cend(), signer.pk_cert.cbegin(),
                     signer.pk_cert.cend());

    // TODO: private_key 하드코딩 되어있음. 설정파일 생성되면 제거할 것
    string private_key = "";
    new_transaction.signature = RSA::doSign(private_key, signature, true);
