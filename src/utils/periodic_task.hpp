...
public:
  ...
  PeriodicTask(T&&io_service, const std::function<void()> &task_function, uint64_t intval = 0, int after = -1) {
  ...
  PeriodicTask(std::shared_ptr<boost::asio::io_service> io_service, const std::function<void()> &task_function, uint64_t intval = 0, int after = -1)  {
    setIoService(io_service);
    setTaskFunction(task_function);
    if(intval != 0) {
      setInterval(intval);
  ...
  void setInterval(uint64_t intval){
    m_interval = intval;
  }
...
private:
  void event_loop(){
    if (m_task_function) {
      if(m_strand_mode) {
        m_io_service->post(m_strand->wrap([this]() {
          m_task_function();
        }));
      } else {
