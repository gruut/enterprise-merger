...
void Storage::write(json &data, const string &what, string &blk_id) {
  ...
  if (what == "block") {
    ...
    auto key = "cert_" + data["nid"].dump();
    auto value = data["x.509"].dump();

    auto status = m_db->Put(m_write_options, key, value);

    handleTrivialError(status);
  } else if (what == "block_hash") {
    auto key = "block_id_" + data["block_id"].dump();
    auto value = data["block_header_hash"].dump();
