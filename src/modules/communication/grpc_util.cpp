...
HeaderController::attachHeader(std::string &compressed_json,
    ...
    if(msg_type == MessageType::MSG_REQ_SSIG || msg_type == MessageType::MSG_ACCEPT)
...
nlohmann::json
HeaderController::getJsonMessage(CompressionAlgorithmType compression_type, std::string &no_header_data) {
    nlohmann::json json_data;
    switch (compression_type) {
    case CompressionAlgorithmType::LZ4: {
        std::string origin_data;
        Compressor::decompressData(no_header_data, origin_data, no_header_data.size());
...
std::string HeaderController::makeHeaderAddedData(MessageHeader &msg_hdr, nlohmann::json &json_obj){
    ...
    std::string header_added_data = attachHeader(json_dump, msg_hdr.message_type, msg_hdr.compression_algo_type);
